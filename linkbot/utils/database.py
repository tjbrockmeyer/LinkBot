...
class Session:
    ...
    def run(self, query: str, **kwargs):
        ...
        return self.s.run(query, **kwargs)

    # MANAGE database calls

    def create_constraints(self):
        self.run("CREATE CONSTRAINT ON (u:User) ASSERT u.id IS UNIQUE")
        self.run("CREATE CONSTRAINT ON (g:Guild) ASSERT g.id IS UNIQUE")

    def create_indexes(self):
        self.run("CREATE INDEX ON :User(birthday)")
        self.run("CREATE INDEX ON :Reminder(at)")
        self.run("CREATE INDEX ON :Quote(ref)")

    def delete_node_with_id(self, node_id):
        """ Delete and detach a node using its ID(). """

        self.run("MATCH (n) WHERE ID(n) = {id} DETACH DELETE n", id=node_id)
    ...
    def create_guild(self, g_id):
        ...
        self.run("MERGE (:Guild {id: {g_id}})", g_id=g_id)
